...
# ===-----------------------------------------------------------------------===
"""This is the referencing.cs module.
...
class CoordinateSystemAxis(IdentifiedObject):
    ...
    Attributes:
        abbreviation (str): The abbreviation used for this coordinate system
            axes. This abbreviation is also used to identify the coordinate in
            a coordinate tuple. The coordinate system axis abbreviation.
        direction (AxisDirection): Direction of this coordinate system axis.
            The coordinate system axis direction.
        unit (...): Returns the unit of measure used for this coordinate
            system axis. The value of a coordinate in a coordinate tuple shall
            be recorded using this unit of measure, whenever those coordinates
            use a coordinate reference system that uses a coordinate system
            that uses this axis.
        minimum_value (float): Returns the minimum value normally allowed for
            this axis, in the unit of measure for the axis. If there is no
            minimum value, then this method returns negative infinity. The
            minimum value, or the negative infinity if none.
        maximum_value (float): Returns the maximum value normally allowed for
            this axis, in the unit of measure for the axis. If there is no
            maximum value, then this method returns positive infinity. The
            maximum value, or the positive infinity if none.
        range_meaning (RangeMeaning): Returns the meaning of axis value range
            specified by the minimum and maximum values. This element shall be
            omitted when both minimum and maximum values are omitted. It may
            be included when minimum and/or maximum values are included. If
            this element is omitted when minimum or maximum values are
            included, the meaning is unspecified. The range meaning, or null
            in none.
...
class CoordinateSystem(IdentifiedObject):
    ...
    Attributes:
        dimension (int): The dimension of the coordinate system.
        axis (tuple[CoordinateSystemAxis, ...]): The axes for this coordinate
            system at the specified dimension. Access each access using a zero
            based index on the tuple.

    """

    dimension: int
    axis: tuple[CoordinateSystemAxis, ...]
...
class CylindricalCS(CoordinateSystem):
    ...
    straight axis perpendicular to the plane spanned by the polar CS.
    """
...
class EllipsoidalCS(CoordinateSystem):
    ...
    geodetic latitude, geodetic longitude, and (in the 3D case) ellipsoidal
    height.
    """
